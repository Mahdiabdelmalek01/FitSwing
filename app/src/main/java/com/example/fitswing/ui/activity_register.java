        ...
        mAuth.createUserWithEmailAndPassword(email, password)
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        ...
                        Intent intent = new Intent(activity_register.this, logins.class);
                        startActivity(intent);
                        finish();

                    } else {
                        Toast.makeText(activity_register.this, "Failed to create user: " + task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
    }
    ...
    private void sendVerificationEmail() {
        ...
        if (user != null) {
            user.sendEmailVerification()
                    .addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
                            Toast.makeText(activity_register.this, "Verification email sent to " + user.getEmail(), Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(activity_register.this, "Failed to send verification email", Toast.LENGTH_SHORT).show();
                        }
                    });
        }
    }
